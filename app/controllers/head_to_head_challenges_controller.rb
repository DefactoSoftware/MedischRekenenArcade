class HeadToHeadChallengesController < AbstractChallengesController
  def create
    ...
    if head_to_head_challenge.save!
      redirect_to challenges_path,
                  notice: I18n.t("head_to_head_challenge.save.success")
    else
      redirect_to challenges_path,
                  notice: I18n.t("head_to_head_challenge.save.fail")
    end
  end

  def show
    @head_to_head_challenge = HeadToHeadChallenge.find(params[:id])
    if @head_to_head_challenge.user_finished(current_user)
      redirect_to my_challenge_path(@head_to_head_challenge)
    else
      super
    end
  end
  ...
  def create_user_challenge
    find_user_challenge || UserChallenge.create(user_challenge_params)
    track_activity(@head_to_head_challenge, "start")
  end

  def find_user_challenge
    @user_challenge = @head_to_head_challenge
                      .user_challenges
  ...
  def head_to_head_challenge_params
    params.require(:head_to_head_challenge).permit(:challenged_id,
                                                   :challenge_id)
  end
  ...
  def calculate_progress
    ...
    amount_problems = Float(@user_challenge
                            .challenge
                            .number_of_problems)
